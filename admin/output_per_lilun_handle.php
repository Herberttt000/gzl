<?php
	...
	//echo $_POST['year'];
	$sql = "SELECT * FROM `lilun` WHERE `xueqi` LIKE \"".$_POST['year']."\"";
	//echo $sql;
...
$objPHPExcel->getActiveSheet()->setCellValue('F1', '课程别名');
$objPHPExcel->getActiveSheet()->setCellValue('G1', '教师院系');
...
while ( $row = mysql_fetch_assoc($result_ll)) {
	...
	$objPHPExcel->getActiveSheet()->setCellValueExplicit('F'.$num, (string)$row['course_alias'],PHPExcel_Cell_DataType::TYPE_STRING);
