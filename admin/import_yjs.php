<?php
	...
	# 列出可删除的学期
	$sql = "SELECT DISTINCT `xueqi` FROM `yanjiusheng`;";
...
<h1 class="page-header">导入研究生数据</h1>
		...
		<div class="alert alert-info alert-dismissible" role="alert">
			<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
			<strong>1.</strong>请注意你的Excel格式是否是xls（xlsx是错误的格式）. 
			<strong>2.</strong>同一学期的同一个教师的工作量必须合并在一行里. 
		</div>
	<article class="col-md-12 maincontent">

		<form action="import_yjs_handle.php" method="post" enctype="multipart/form-data">
		...
		<form class="form-horizontal" action="./import_yjs_delete_handle.php" method="post">
						<fieldset>
							<!-- Select Basic -->
							<h3>选择要删除的学期</h3>
