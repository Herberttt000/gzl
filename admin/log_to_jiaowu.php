...
$xxx = vlogin("http://202.118.201.228/academic/j_acegi_security_check","j_username=".$j_username."&j_password=".$j_password."&j_captcha=".$j_captcha);
//print_r($xxx);
$data = vget("http://202.118.201.228/academic/calendarinfo/viewCalendarInfo.do");
$name = vget("http://202.118.201.228/academic/showHeader.do");
//echo $data;
preg_match("/<\/\span>(.+)/", $data, $ma);
//print_r($ma);
$res  = preg_match("/\((\d+)\)/", $name, $ma2);
//print_r($ma2);
if($res == 1 && $ma2[1]!=0) {
    echo "验证成功！";
    ...
    echo "<br /> ".$ma2[1];
    //$ma2[1] = "0401361";
    echo "<a href=\"../get_info_pro.php?xueqi=20141&teacher_id=".$ma2[1]."&token=".md5($ma2[1].date("d"))."\">查询工作量</a>";
}else{
    echo "验证失败！";
}
?>