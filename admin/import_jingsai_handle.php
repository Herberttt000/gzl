<?php
	...
	*/
    ...
    if(!$_SESSION['import_jingsai']) {
        header("location: ./error.php?txt="."您没有导入竞赛信息的权限.");
        exit();
    }
 ...
 else
 {
  ...
  exit();
	...
	*/
    ...
    // 	$sql = "select count(*) from `jingsai` where `teacher_id`=\"".$output[$row]['teacher_id']."\"
    ...
    for($row = 2; $row <= $allRow; $row++) {
        ...
        $sql = "INSERT INTO `jingsai` (xueqi, teacher_id,
