<?php
	...
	*/
    ...
    if(!$_SESSION['import_jiaowu']) {
        header("location: ./error.php?txt="."您没有导入教务津贴信息的权限.");
        exit();
    }
include "./header.php";
	...
	*/
    ...
    for($currentRow = 2;$currentRow<=$allRow;$currentRow++){ 
    	...
    	$output[$currentRow]['teacher_id'] = (String)$currentSheet->getCell('B'.$currentRow)->getValue(); 
    	$output[$currentRow]['teacher_name'] = (String)$currentSheet->getCell('C'.$currentRow)->getValue();  
    	$output[$currentRow]['yuanyin'] = (String)$currentSheet->getCell('D'.$currentRow)->getValue();
    	$output[$currentRow]['jiaofen'] = (float)$currentSheet->getCell('E'.$currentRow)->getValue();
    ...
    for($currentRow = 2; $currentRow <= $allRow; $currentRow++) {
        ...
        if($teachername != $id2teacher[$xueqi_row][$teacherid]) {
            $error .= "教师号为“".$teacher_id."”的教师,姓名应为“".$id2teacher[$teacherid]."”.<br />";
            $error .= "姓名为“".$teachername."”的教师，教师号应为“".$teacher2id[$teachername]."”.<br />";
            echo $error;
            flush();
            $haserror = 1;
        }
 ...
 //    	$sql = "select count(*) from `jiaowu` where `teacher_id`=\"".$output[$row]['teacher_id']."\"
 ...
 //    }
    ...
    for($row = 2; $row <= $allRow; $row++) {
    		...
    		$sql = "INSERT INTO `jiaowu` (xueqi, teacher_id,
    			teacher_name,yuanyin,jiaofen
    			) VALUES (\"".
    			$output[$row]['xueqi']."\",\"".
