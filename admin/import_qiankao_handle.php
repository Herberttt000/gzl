<?php
	...
	*/
    ...
    if(!$_SESSION['import_qiankao']) {
        header("location: ./error.php?txt="."您没有导入欠考信息的权限.");
        exit();
    }
include "./header.php";
	...
	*/
    ...
    for($currentRow = 2;$currentRow<=$allRow;$currentRow++){ 
    	...
    	$output[$currentRow]['jiaofen'] = (float)$currentSheet->getCell('D'.$currentRow)->getValue();
 ...
 //    	$sql = "select count(*) from `qiankao` where `teacher_id`=\"".$output[$row]['teacher_id']."\"
 ...
 //    }
    ...
    for($row = 2; $row <= $allRow; $row++) {
    		$sql = "INSERT INTO `qiankao` (xueqi, teacher_id,
    			teacher_name,jiaofen
    			) VALUES (\"".
    			$output[$row]['xueqi']."\",\"".
