...
# delete the old data.
$sql = "DELETE FROM lilun
WHERE xueqi={$_GET['xueqi']};";
$result = mysql_query($sql);
if(!$result){
    die(mysql_error());
}
...
AND `course_name`!=\"体育-Ⅱ\"
AND `course_name`!=\"体育-Ⅲ\"
AND `course_name`!=\"体育-Ⅳ\"
...
set_time_limit(0);
    ...
    */
    while($row = mysql_fetch_assoc($ll_result)) {
        /*
        $len = strlen( $row['teacher_id'] );
        if ( $len < 7 && $len > 0 && is_numeric($row['teacher_id'])) {
            $row['teacher_id'] = $leadingZero[$len] . $row['teacher_id'];
        }
        */

        if(!isset( $id2zc[ $row['teacher_id'] ]) ) {
            echo "teacher who's id is {$row['teacher_id']} be ignored!<br>";
            continue;
        }
        $cnt++;
        ?>
    ...
    else if($num_of_people >= $xishu_rows['M0']//Update by Houbaron 20161106
...
$renshuxishu = $K;
    ...
    //                       2016-11-06                             //
    //////////////////////////////////////////////////////////////////
    //don't forget to modify modify_lilun_per_handle.php
    $zhuanyekexishu = 0;
    if (strlen($row['course_id']) == 9) {
    ...
    if(!isset($zc_rows[$id2zc[$row['teacher_id']]])) {
        echo $row['teacher_id'];
        echo "不存在“".$id2zc[$row['teacher_id']]."”职称，请在系统中添加该职称信息.</br>";
        continue;
    }
...
<?php
echo "<strong>all: $count_of_calculated </strong>";
