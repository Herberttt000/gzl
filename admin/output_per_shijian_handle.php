...
//echo $_POST['year'];
//$cols = "id, shijian.xueqi, shijian.teacher_id, shijian.teacher_name, teacher_zc, shijian_name, course_id, shijian_type, zhichengxishu, zhoushu, num_of_p, banji, didian, guocheng, jiaofen, teachers.xueyuan";
$sql = "SELECT shijian.*, teachers.xi FROM `shijian` INNER JOIN teachers ON teachers.xueqi=\"".$_POST['year']."\" AND shijian.xueqi=\"".$_POST['year']."\" AND shijian.teacher_id=teachers.teacher_id";
...
// 设置工作薄名称
$objPHPExcel->getActiveSheet()->setTitle(iconv('gbk', 'utf-8', 'daochu'));
// 设置默认字体和大小
...
$objPHPExcel->getActiveSheet()->setCellValue('Q1', '教师院系');
...
while ($row = mysql_fetch_assoc($result_sj)) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('Q'.$num, (string)$row['xi'],PHPExcel_Cell_DataType::TYPE_STRING);
    $num++;
}
