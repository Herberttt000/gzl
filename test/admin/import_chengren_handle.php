<?php
	...
	*/
    ...
    if(!$_SESSION['import_chengren']) {
        header("location: ./error.php?txt="."您没有导入成人教育信息的权限.");
        exit();
    }
include "./header.php";
	...
	*/
    ...
    for($currentRow = 2;$currentRow<=$allRow;$currentRow++){ 
    	...
    	$output[$currentRow]['shijianzhehe'] = (float)$currentSheet->getCell('F'.$currentRow)->getValue();
    ...
    for($currentRow = 2; $currentRow <= $allRow; $currentRow++) {
        ...
        if($teachername != $id2teacher[$xueqi_row][$teacherid]) {
          $error = "";
            if(isset($id2teacher[$xueqi_row][$teacherid])) $error .= "教师号为“".$teacherid."”的教师,姓名应为“".$id2teacher[$xueqi_row][$teacherid]."”.<br />";
 ...
 //    	$sql = "select count(*) from `chengren` where `teacher_id`=\"".$output[$row]['teacher_id']."\"
 ...
 //    }
    ...
    for($row = 2; $row <= $allRow; $row++) {
    		...
    		$sql = "INSERT INTO `chengren` (xueqi, teacher_id,
    			teacher_name,yuanshi,zhehe,shijianzhehe
    			) VALUES (\"".
    			$output[$row]['xueqi']."\",\"".
