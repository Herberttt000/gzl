<?php
	...
	*/
    ...
    if(!$_SESSION['import_yanjiusheng']) {
        header("location: ./error.php?txt="."您没有导入研究生信息的权限.");
        exit();
    }
...
include "./header.php";
	...
	*/
    ...
    for($currentRow = 2;$currentRow<=$allRow;$currentRow++){ 
    	...
    	$output[$currentRow]['zhidao'] = (float)$currentSheet->getCell('F'.$currentRow)->getValue();
        $output[$currentRow]['mubiao'] = (float)$currentSheet->getCell('G'.$currentRow)->getValue();
        $output[$currentRow]['zdjs'] = (float)$currentSheet->getCell('H'.$currentRow)->getValue();
        $output[$currentRow]['isshuodao'] = str_replace(' ','',(String)$currentSheet->getCell('I'.$currentRow)->getValue());
    }
 ...
 //    	$sql = "select count(*) from `yanjiusheng` where `teacher_id`=\"".$output[$row]['teacher_id']."\"
 ...
 //    }
    ...
    for($row = 2; $row <= $allRow; $row++) {
    	$sql = "INSERT INTO `yanjiusheng` (xueqi, teacher_id,
    			teacher_name,yuanshi,zhehe,zhidao,isshuodao,mubiao,zdjs
    			) VALUES (\"".
    			$output[$row]['xueqi']."\",\"".
